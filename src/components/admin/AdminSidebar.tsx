...
const NAV = [
  ...
  {
    group: 'Settings',
    items: [{ href: '/admin/settings', label: 'Settings', icon: Settings, disabled: true }],
  },
];
    ...
    <aside className="hidden md:block col-span-3 lg:col-span-2 p-4 space-y-5 bg-white border-r">
      ...
      {NAV.map((g) => (
        <div key={g.group} className="space-y-2">
          <div className="px-2 text-xs uppercase tracking-wide text-slate-500">
            {g.group}
          </div>
            ...
            {g.items.map((n) => {
              ...
              return (
                <Link
                  key={n.href}
                  href={n.href}
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl ${cls}`}
